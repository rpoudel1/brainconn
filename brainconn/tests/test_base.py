...
def test_threshold_absolute():
    x = load_sample()
    x = bc.threshold_absolute(x, 2.1)
    assert np.allclose(np.sum(x), 13280.17768104)


def test_strengths_und():
    x = load_sample()
    s = bc.strengths_und(x)
    assert np.allclose(np.sum(x), 38967.38702018)


def test_degrees_und():
    x = load_sample()
    s = bc.degrees_und(bc.threshold_proportional(x, .26))
    assert np.sum(s) == 4916


def test_binarize():
    x = load_sample()
    s = bc.binarize(bc.threshold_proportional(x, .41))
    assert np.sum(s) == 7752


def test_normalize():
    x = load_sample()
    s = bc.normalize(bc.threshold_proportional(x, .79))
    assert np.allclose(np.sum(s), 3327.96285964)


def test_invert():
    x = load_sample()
    s = bc.invert(bc.threshold_proportional(x, .13))
