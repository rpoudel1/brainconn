...
def test_consensus():
    ...
    ci = bc.consensus_und(x, .1, reps=50)
    print(np.max(ci), 4)
    assert np.max(ci) == 4
    _, q = bc.modularity_und(x, kci=ci)
    print(q, 0.27)
...
def test_transitivity_wd():
    x = load_directed_low_modularity_sample(thres=.45)
    t = bc.transitivity_wd(x)
    print(t, 1.30727748)
    assert np.allclose(t, 1.30727748)


def test_cluscoef_bu():
    x = bc.binarize(load_sample(thres=.17), copy=False)
    cc = bc.clustering_coef_bu(x)
